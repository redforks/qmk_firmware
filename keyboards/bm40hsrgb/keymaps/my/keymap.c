...
void dance_pp_reset(qk_tap_dance_state_t *state, void *user_data) {
    if (state->count == 1) {
        unregister_code(KC_P);
    }
}

void dance_rr_finished(qk_tap_dance_state_t *state, void *user_data) {
    switch (state->count) {
      case 1:
        register_code(KC_R);
        break;        
...
void dance_rr_reset(qk_tap_dance_state_t *state, void *user_data) {
    if (state->count == 1) {
        unregister_code(KC_R);
    }
}

void dance_ff_finished(qk_tap_dance_state_t *state, void *user_data) {
    switch (state->count) {
      case 1:
        register_code(KC_F);
        break;        
...
void dance_ff_reset(qk_tap_dance_state_t *state, void *user_data) {
    if (state->count == 1) {
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	[0] = LAYOUT_planck_mit(KC_SLSH, LT(5,KC_QUOT), KC_COMM, KC_DOT, TD(TD_PP), KC_Y, TD(TD_FF), KC_G, KC_C, TD(TD_RR), KC_L, KC_BSPC, KC_TAB, LGUI_T(KC_A), LALT_T(KC_O), LCTL_T(KC_E), LSFT_T(KC_U), KC_I, KC_D, RSFT_T(KC_H), LCTL_T(KC_T), LALT_T(KC_N), LGUI_T(KC_S), KC_MINS, KC_ESC, LT(6,KC_SCLN), KC_Q, KC_J, KC_K, KC_X, KC_B, KC_M, KC_W, KC_V, KC_Z, KC_ENT, MO(4), KC_NO, KC_NO, KC_NO, MO(2), LT(1,KC_SPC), MO(2), KC_NO, KC_CAPS, KC_NO, MO(3)),
